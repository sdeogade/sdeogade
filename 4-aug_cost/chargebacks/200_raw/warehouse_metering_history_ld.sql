...
(
    select
        ...
        ,s.*
    from
        &{l_raw_db}.&{l_raw_schema}.warehouse_metering_history_stg s
)
,l_deduped as
(
    select
...
where
    ...
    (
        select dw_warehouse_metering_history_shk from &{l_raw_db}.&{l_raw_schema}.warehouse_metering_history where start_time >= $l_start_dt and start_time < $l_end_dt
    )
order by
