...
--
insert into
    &{l_raw_db}.&{l_raw_schema}.database_storage_usage_history
with l_stg as
(
    select
        ...
        ,s.*
    from
        &{l_raw_db}.&{l_raw_schema}.database_storage_usage_history_stg s
)
,l_deduped as
(
    select
...
where
    ...
    (
        select dw_database_storage_usage_history_shk from &{l_raw_db}.&{l_raw_schema}.database_storage_usage_history where usage_date >= $l_start_dt and usage_date < $l_end_dt
    )
order by
