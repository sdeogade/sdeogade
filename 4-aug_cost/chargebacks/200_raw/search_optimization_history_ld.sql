...
--
insert into
    &{l_raw_db}.&{l_raw_schema}.search_optimization_history
with l_stg as
(
    select
        ...
        ,s.*
    from
        &{l_raw_db}.&{l_raw_schema}.search_optimization_history_stg s
)
,l_deduped as
(
    select
...
where
    ...
    (
        select dw_search_optimization_history_shk from &{l_raw_db}.&{l_raw_schema}.search_optimization_history where start_time >= $l_start_dt and start_time < $l_end_dt
    )
order by
