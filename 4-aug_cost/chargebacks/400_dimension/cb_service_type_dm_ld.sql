...
--
merge into &{l_pl_db}.&{l_pl_schema}.cb_service_type_dm t using
(
    with l_stg as
    (
        select
            ...
            ,s.dw_update_ts              
        from
            &{l_raw_db}.&{l_raw_schema}.cb_service_type_lkp s
    )
    ,l_deduped as
    (
        select
    ...
    from
        l_deduped s
        left join &{l_pl_db}.&{l_pl_schema}.cb_service_type_dm t on
