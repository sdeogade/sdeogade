...
--
create or replace temporary table &{l_il_db}.&{l_il_schema}.fcst_period_in as
with l_range_month as
(
    select
         &{l_common_db}.&{l_common_schema}.date_sid_f( event_month_dt )    as dw_event_date_sid
        ,event_month_dt
        ,case
...
(
    ...
    from
        &{l_il_db}.&{l_il_schema}.cb_account_consumption_ms
    group by
        1
)
,l_account_month as
...
from
    l_account_month lam
    left join &{l_il_db}.&{l_il_schema}.cb_account_consumption_ms cacm on
...
--
call &{l_common_db}.&{l_common_schema}.fcst_linear_trend_sp( '&{l_il_db}.&{l_il_schema}.fcst_period_in', '&{l_il_db}.&{l_il_schema}.fcst_period_out' );
...
begin;
    ...
    --
    delete from
        &{l_il_db}.&{l_il_schema}.cb_account_consumption_fcst_ms
    where
    ...
    from
        &{l_il_db}.&{l_il_schema}.fcst_period_in fpi
        join &{l_il_db}.&{l_il_schema}.fcst_period_out fpo on
                fpo.partition_key = fpi.partition_key
            and fpo.period_dt     = fpi.period_dt
    where
        not exists
        (
            select 1 from &{l_il_db}.&{l_il_schema}.cb_account_consumption_fcst_ms where event_month_dt >= $l_start_dt and event_month_dt < $l_end_dt
        )
    ;
